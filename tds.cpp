...
static int query_message_handler(const TDSCONTEXT* context, TDSSOCKET* sock, TDSMESSAGE* msg) {
...
static int query_error_handler(const TDSCONTEXT* context, TDSSOCKET* sock, TDSMESSAGE* msg) {
	throw runtime_error(msg->message);
...
void TDSQuery::add_param2(unsigned int i, const binary_string& param) {
	TDSPARAMINFO* params;
	...
	dyn->params = params;
	tds_set_param_type(tds.sock->conn, params->columns[i], SYBVARBINARY);
	...
	tds_alloc_param_data(params->columns[i]);
	memcpy(params->columns[i]->column_data, param.s.c_str(), param.s.size());
}
...
TDSRET TDSConn::bcp_get_column_data(TDSCOLUMN* bindcol, int offset) {
	...
	if (v.is_null() && bindcol->column_nullable) {
		bindcol->bcp_column_data->datalen = 0;
...
void TDSConn::bcp(const string& table, const vector<string>& np, const vector<vector<nullable<string>>>& vp) {
	memset(&bcpinfo, 0, sizeof(bcpinfo));
	...
	if (!tds_dstr_copyn(&bcpinfo.tablename, table.c_str(), table.length()))
	...
	try {
		...
		auto gcd = [](TDSBCPINFO* bcpinfo, TDSCOLUMN* bindcol, int offset) {
			auto tds = CONTAINING_RECORD(bcpinfo, TDSConn, bcpinfo);

			return tds->bcp_get_column_data(bindcol, offset);
		};

		for (unsigned int i = 0; i < vp.size(); i++) {
			if (TDS_FAILED(tds_bcp_send_record(sock, &bcpinfo, gcd, nullptr, i)))
